...
int io61_flush_helper(io61_file *f); // declare the function so it can be found
struct io61_file
{
    ...
    // Positioned mode
    std::atomic<bool> dirty = false; // fix data races, make type atomic
    bool positioned = false;         // is cache in positioned mode?

    std::mutex m;
    std::condition_variable_any cv;    // intialize cv and m to control locking
    std::map<off_t, off_t> file_locks; // intialize a map of the ranges of the locks
    // for mmap
    unsigned char *file_data; // pointer that mmap returns
    size_t file_size;         // tracks the size of the mmapped file
...
ssize_t io61_read(io61_file *f, unsigned char *buf, size_t sz)
{
    ...
    while (pos < sz)
    {
        ...
        memcpy(&buf[pos], &f->cbuf[f->pos_tag - f->tag], mem_size);               // taking the input in the cache and copying it to the buffer in physical memory
...
{
    std::unique_lock guard(f->m); // coarse grained lock this file
    return io61_flush_helper(f);  // avoid deadlocks by running flush through a helper function other than flush function, doesn't always coarse grain lock
}
...
int io61_try_lock(io61_file *f, off_t start, off_t len, int locktype)
{
    ...
    f->file_locks.insert({start, start + len}); // use map to track what is being locked
    return 0;
}
...
int io61_unlock(io61_file *f, off_t start, off_t len)
{
    ...
    if (r == 0)
    { // check if nothing was erased, then report an error
        return -1;
    }
